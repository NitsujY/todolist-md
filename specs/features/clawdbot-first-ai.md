# Clawdbot-First AI Architecture Spec

## Overview
This spec defines the redesigned AI feature set where **Clawdbot is the primary AI engine**, and the SPA provides lightweight capture/annotation UI only.

## Design Principles
1. **Clawdbot as AI brain** - All LLM analysis happens in Clawdbot
2. **SPA as capture interface** - Fast, offline-capable text input
3. **Markdown as communication layer** - Clawdbot reads/writes markdown annotations
...
## Added: Clawdbot Integration Markers
...
### 2. Clawdbot Suggestions Section
Clawdbot appends analyzed tasks:
...
- [ ] Complete landing page redesign #frontend due:2026-02-05
  > Subtasks: mockup approval, HTML structure, styling, responsive, deploy
  
- [ ] Fix auth null pointer bug #backend #urgent
  > Location: src/auth.ts line 42
...
### 3. Task Annotations
Clawdbot can add inline comments to existing tasks:

```markdown
- [ ] Deploy v2.0 to production
  > <!-- Clawdbot: Blocked by failing integration tests. Run `npm test` first. -->
```
