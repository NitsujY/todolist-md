...
const FILTER_DIR='outputs/todolist-md/filter_records'
const SUG_PATH='outputs/todolist-md/llm_suggestions_apply.json'
if(!fs.existsSync(SUG_PATH)){console.error('missing suggestions file'); process.exit(1)}
// pick the latest filter record for the file
const files = fs.readdirSync(FILTER_DIR).filter(f=>f.endsWith('.json')).sort().reverse()
if(files.length===0){console.error('no filter records'); process.exit(1)}
const recPath = `${FILTER_DIR}/${files[0]}`
const records = JSON.parse(fs.readFileSync(recPath,'utf8'))
const sugg = JSON.parse(fs.readFileSync(SUG_PATH,'utf8'))
let items = []
if(sugg.fileSuggestions && sugg.fileSuggestions[0]) items = sugg.fileSuggestions[0].suggestions || sugg.fileSuggestions[0].filtered_and_normalized_suggestions || []
else if(sugg.suggestions) items = sugg.suggestions
let fileId = (sugg.fileSuggestions && sugg.fileSuggestions[0] && sugg.fileSuggestions[0].fileId) || sugg.fileId || null
if(!fileId){
  // fallback to known vyond fileId
  fileId = '1W4ua8gXc36v9onOL3UkppzQuD4BaiTDE'
  console.warn('fileId not found in suggestions; falling back to',fileId)
}
...
(async ()=>{
  ...
  // build map of originals -> suggestion
  const map = {}
  for(const it of items){ map[it.original]=it }
  for(const rec of records){
    if(rec.decision!=='insert') continue
    const orig = rec.original
    const it = map[orig]
    if(!it) continue
    const suggested = it.suggested_text || it.suggestion || ''
    const title = suggested.replace(/^\- \[ \]\s*/,'').split('\n')[0]
    // find best match
    let bestIdx=null,bestScore=-1
    for(let i=0;i<lines.length;i++){ const a = normalize(orig), b=normalize(lines[i]);
      const sim = 1 - (levenshtein(a,b)/Math.max(a.length,1)); if(sim>bestScore){bestScore=sim; bestIdx=i}}
